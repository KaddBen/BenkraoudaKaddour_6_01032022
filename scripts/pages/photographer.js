...
SectionAllImg.forEach((img) => {
  img.addEventListener("click", () => {
    ...
    if (video) {
      ...
      lightBox.appendChild(close);
      const lightboxAllImg = document.querySelectorAll(".lightbox_img");
      for (let o = 0; o < lightboxAllImg.length - 1; o++) {
        const currentImg = lightboxAllImg[o];
        currentImg.remove();
      }
      const lightboxAllVideo = document.querySelectorAll(".lightbox_vid");
...
let sectionAllDiv = document.querySelectorAll(".media");
sectionAllDiv.forEach((div) => {
  div.addEventListener("keydown", (e) => {
    if (e.keyCode === 32) {
      let divImg = div.firstChild;
      let imgId = divImg.id;
      let mediaId = medias.find((media) => media.id.toString() === imgId);
      const { name } = responseId;
      const { image, id, video } = mediaId;
...
dialog.addEventListener("keydown", (e) => {
  let firstFocusableElement = document.querySelector(".lightbox_close");
  let lastFocusableElement = document.querySelector(".fa-solid.fa-angle-right");
  let arrowleft = document.querySelector(".fa-solid.fa-angle-left");
  let vid = document.querySelector("lightbox_vid");
...
let SubmitBtn = document.getElementsByName("Send");
let Sent = document.querySelector(".sent");
let MessageForm = document.querySelector("textarea");
